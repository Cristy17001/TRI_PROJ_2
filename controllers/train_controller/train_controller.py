    ...
    try:
        ...
        if os.path.exists(model_path):
            ...
            model = PPO(
                ...
                learning_rate=3e-4,  # Aumentado para melhor aprendizado
                n_epochs=10,
                gamma=0.99,
                gae_lambda=0.95,
                ent_coef=0.01,  # Aumentado para mais exploração
                clip_range=0.2,
                max_grad_norm=0.5  # Adicionar gradient clipping
            )
        ...
        model = PPO(
            ...
            n_steps=n_steps,
            batch_size=batch_size,
            learning_rate=3e-4,  # Learning rate adequado para recompensas normalizadas
            n_epochs=10,
            gamma=0.99,
            gae_lambda=0.95,
            ent_coef=0.01,  # Entropia para exploração
            clip_range=0.2,
            max_grad_norm=0.5,  # Gradient clipping para estabilidade
            vf_coef=0.25  # Reduzir peso do value function para estabilizar loss
        )
